...
namespace DirRX.EssPlatformSolution
{
  partial class EmployeeSharedHandlers
  {
    ...
    public virtual void InheritFromBusinessUnitDirRXChanged(Sungero.Domain.Shared.BooleanPropertyChangedEventArgs e)
    {
      if (e.NewValue != e.OldValue)
      {
        _obj.NeedNotifyNewHRAssignmentDirRX = DirRX.EssPlatformSolution.Employee.NeedNotifyNewHRAssignmentDirRX.No;
        _obj.NeedNotifyExpiredHRAssignmentDirRX = DirRX.EssPlatformSolution.Employee.NeedNotifyExpiredHRAssignmentDirRX.No;
        _obj.NeedNotifyHRRepeatDirRX = DirRX.EssPlatformSolution.Employee.NeedNotifyHRRepeatDirRX.No;
      }
    }
    ...
    public override void PersonChanged(Sungero.Company.Shared.EmployeePersonChangedEventArgs e)
    {
      ...
      if (_obj.Status == Employee.Status.Active && newPerson != null && newPerson != e.OldValue)
        Functions.Employee.Remote.FillPersonalOfficeProperties(_obj);
      Functions.Employee.Remote.ClearPersonalPhoneAndStatus(_obj);
    }
