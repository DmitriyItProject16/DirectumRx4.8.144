...
namespace Sungero.Docflow.Server
{
  partial class PersonalSettingFunctions
  {
    ...
    {
      var personalSettings = PersonalSettings.GetAll(s => Equals(s.Employee, employee)).OrderBy(s => s.Id).FirstOrDefault();
    ...
    public static bool GetShowRegistrationPaneParam(IEmployee employee)
    {
      ...
      var showRegPane = GetOrCreatePersonalSettings(employee).ShowRegPane;
      if (showRegPane != null)
