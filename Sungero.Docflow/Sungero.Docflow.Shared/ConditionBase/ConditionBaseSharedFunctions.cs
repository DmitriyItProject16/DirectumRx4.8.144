...
namespace Sungero.Docflow.Shared
{
  partial class ConditionBaseFunctions
  {
    ...
    public virtual string GetNotUsableConditionHint()
    {
      ...
      return ApprovalRuleBases.Resources.ConditionNotSupportedByThisDocumentFormat(localizedConditionType);
    }
    ...
    public virtual Structures.ConditionBase.ConditionResult CheckAmountIsMore(IOfficialDocument document, IApprovalTask task)
    {
      ...
      if (contractual != null)
      {
        var totalAmount = contractual.TotalAmount.HasValue ? contractual.TotalAmount.Value : 0d;
        return Structures.ConditionBase.ConditionResult.Create(this.CheckAmount(totalAmount), string.Empty);
      }
    ...
    {
      if (document != null)
        return Structures.ConditionBase.ConditionResult.Create(Functions.OfficialDocument.IsProjectDocument(document, new List<long>()), string.Empty);
