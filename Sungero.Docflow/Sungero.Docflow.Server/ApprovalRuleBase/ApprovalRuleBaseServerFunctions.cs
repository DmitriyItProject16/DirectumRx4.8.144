  ...
  partial class ApprovalRuleBaseFunctions
  {
                                              ...
                                              Structures.ApprovalRuleBase.RouteStep conditionStep)
    {
      ...
      var allConditions = ruleConditions
        .Where(x => routeSteps.Contains(x.RouteStep.StepNumber) && x.RouteStep.StepNumber != conditionStep.StepNumber)
        .Select(x => Structures.ApprovalRuleBase.ConditionRouteStep.Create(Structures.ApprovalRuleBase.RouteStep.Create(x.RouteStep.StepNumber,
                                                                                                                        route.First(r => r.StepNumber == x.RouteStep.StepNumber).Branch),
                                                                           x.ConditionType))
        .ToList();
      ...
      {
        var amountConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.AmountIsMore).Select(x => x.RouteStep).ToList();
      ...
      {
        var currencyConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.Currency).Select(x => x.RouteStep).ToList();
      ...
      {
        var nonresidentConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.Nonresident).Select(x => x.RouteStep).ToList();
      ...
      {
        var projectConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.ProjectDocument).Select(x => x.RouteStep).ToList();
      ...
      {
        var deliveryConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.DeliveryMethod).Select(x => x.RouteStep).ToList();
      ...
      {
        var documentKindConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.DocumentKind).Select(x => x.RouteStep).ToList();
      ...
      {
        var conditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.RolesComparer).Select(x => x.RouteStep).ToList();
      ...
      {
        var conditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.RoleEmpComparer).Select(x => x.RouteStep).ToList();
      ...
      {
        var conditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.EmployeeInRole).Select(x => x.RouteStep).ToList();
        possibleStage = this.CheckRoleEmpComparerConditions(conditions, conditionStep);
      }
      ...
      {
        var signedByCounterpartyConditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.SignedByCParty).Select(x => x.RouteStep).ToList();
      ...
      {
        var conditions = allConditions.Where(x => x.ConditionType == Docflow.ConditionBase.ConditionType.HasAddenda).Select(x => x.RouteStep).ToList();
    ...
    {
      return route.Where(e => _obj.Conditions.Any(x => Equals(x.Number, e.StepNumber) && Equals(x.Condition.ConditionType, conditionType))).ToList();
    }
    ...
    public virtual List<Structures.ApprovalRuleBase.StagesIncorrectRoles> CheckImpossibleRoles(List<int> route, bool showErrors, List<Enumeration?> verifiedRoles)
    {
      ...
      {
        var failedStages = Functions.ApprovalRuleBase.GetStagesWithProjectRolesUnderNonProjectBranches(_obj, route);
        if (failedStages.Any())
        {
          if (showErrors)
          {
    ...
    /// </summary>
    /// <param name="route">Список номеров этапов.</param>
    /// <returns>Список этапов с проектными ролями, находящихся в ветках для документа без проекта.</returns>
    ...
    {
      var badStagesWithProjectRoles = new List<Structures.ApprovalRuleBase.StageWithUnsupportedRole>();
          ...
          if (condition != null && condition.Condition.ConditionType == Docflow.ConditionBase.ConditionType.ProjectDocument)
          {
            ...
            for (int j = i + 1; j < route.Count; j++)
            {
              ...
              if (Functions.ApprovalStage.HasRole(stage.Stage, Docflow.ApprovalRoleBase.Type.ProjectManager))
                badStagesWithProjectRoles.Add(Structures.ApprovalRuleBase.StageWithUnsupportedRole.Create(stage, Docflow.ApprovalRoleBase.Type.ProjectManager));
              
              if (Functions.ApprovalStage.HasRole(stage.Stage, Docflow.ApprovalRoleBase.Type.ProjectAdmin))
                badStagesWithProjectRoles.Add(Structures.ApprovalRuleBase.StageWithUnsupportedRole.Create(stage, Docflow.ApprovalRoleBase.Type.ProjectAdmin));
            }
          }
        }
      }
      return badStagesWithProjectRoles;
    }
                                          ...
                                          int targetStageFalseCase)
    {
      ...
      if (rule.Conditions.Any())
        maxConditionNumber = rule.Conditions.Select(x => x.Number).Max() ?? 0;
    ...
    public static long GetForReapprovingAssignmentResultId(long taskId)
    {
      ...
      using (var command = SQL.GetCurrentConnection().CreateCommand())
      {
        ...
        long result = 0;
        return long.TryParse(resultId, out result) ? result : 0;
      }
    }
    ...
    private static bool NeedShowSendingStage(IApprovalTask task, Structures.Module.DefinedApprovalStageLite stage, List<Structures.Module.DefinedApprovalStageLite> stages)
    {
      ...
      foreach (var assignment in assignments.ToList())
      {
        ...
        if (printingAsg != null && printingAsg.CollapsedStagesTypesPr.Any(x => x.StageType == StageType.Sending))
        {
          var collapsed = printingAsg.CollapsedStagesTypesPr.ToList();
          var stageIndex = collapsed.IndexOf(collapsed.Single(x => x.StageType == StageType.Print));
        ...
        if (registrationAsg != null && registrationAsg.CollapsedStagesTypesReg.Any(x => x.StageType == StageType.Sending))
        {
          var collapsed = registrationAsg.CollapsedStagesTypesReg.ToList();
          var stageIndex = collapsed.IndexOf(collapsed.Single(x => x.StageType == StageType.Register));
        ...
        if (sendingAsg != null && sendingAsg.CollapsedStagesTypesSen.Any(x => x.StageType == StageType.Sending))
        {
          var collapsed = sendingAsg.CollapsedStagesTypesSen.ToList();
          var stageIndex = collapsed.IndexOf(collapsed.Single(x => x.StageType == StageType.Sending));
        ...
        if (signingAsg != null && signingAsg.CollapsedStagesTypesSig.Any(x => x.StageType == StageType.Sending))
        {
          var collapsed = signingAsg.CollapsedStagesTypesSig.ToList();
          var stageIndex = collapsed.IndexOf(collapsed.Single(x => x.StageType == StageType.Sign || x.StageType == Docflow.ApprovalSigningAssignmentCollapsedStagesTypesSig.StageType.ConfirmSign));
        ...
        if (reviewAsg != null && reviewAsg.CollapsedStagesTypesRe.Any(x => x.StageType == StageType.Sending))
        {
          var collapsed = reviewAsg.CollapsedStagesTypesRe.ToList();
          var stageIndex = collapsed.IndexOf(collapsed.Single(x => x.StageType == StageType.Review || x.StageType == Docflow.ApprovalReviewAssignmentCollapsedStagesTypesRe.StageType.ReviewingResult));
        ...
        if (executionAsg != null && executionAsg.CollapsedStagesTypesExe.Any(x => x.StageType == StageType.Sending))
        {
          var collapsed = executionAsg.CollapsedStagesTypesExe.ToList();
          var stageIndex = collapsed.IndexOf(collapsed.Single(x => x.StageType == StageType.Execution));
    ...
    {
      var document = task.DocumentGroup.OfficialDocuments.FirstOrDefault();
      var lastReapprovalAssignmentId = GetForReapprovingAssignmentResultId(task.Id);
      var assignmentsByTask = Assignments.GetAll().Where(a => a.Task.Id == task.Id).ToList();
      var lastReworkAssignmentIndex = assignmentsByTask.FindIndex(a => a.Id == lastReapprovalAssignmentId);
      ...
      var needSignByCounterparty = document.ExchangeState == Docflow.OfficialDocument.ExchangeState.SignAwaited ||
      ...
      return assignmentsByTask.Any(a => ApprovalCheckReturnAssignments.Is(a) && ApprovalCheckReturnAssignments.As(a).StageNumber == stage.Number) ||
        (statusInfo.IsNext && needSignByCounterparty);
    }
    ...
    public static IQueryable<IApprovalRuleBase> GetApprovalRulesByDocumentKind(IDocumentKind documentKind)
    {
      return ApprovalRuleBases.GetAll(r => r.Status == Docflow.ApprovalRuleBase.Status.Active || r.Status == Docflow.ApprovalRuleBase.Status.Draft)
        .Where(r => r.DocumentKinds.Any(k => k.DocumentKind.Id == documentKind.Id) || r.Conditions.Any(x => x.Condition.ConditionDocumentKinds.Any(d => d.DocumentKind.Id == documentKind.Id)));
    }
    ...
    public static IQueryable<IApprovalRuleBase> GetRulesByParamsIds(List<IApprovalRuleBase> activeRules, Enumeration? documentFlow,
                                                                    long businessUnitId, long documentKindId, long departmentId, long documentGroupId)
    {
      if (documentFlow == null)
    ...
    {
      return route.Where(e => _obj.Conditions.Any(x => Equals(x.Number, e.StepNumber) && x.Condition.ConditionType == conditionType))
        .ToList();
    }
  }
}