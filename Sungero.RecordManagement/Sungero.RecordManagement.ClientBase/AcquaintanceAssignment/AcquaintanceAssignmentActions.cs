...
namespace Sungero.RecordManagement.Client
{
  partial class AcquaintanceAssignmentActions
  {
    public virtual void OpenDocument(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      var acquaintanceVersionNumber = Functions.AcquaintanceTask.GetAcquaintanceVersionNumber(task);
      Functions.AcquaintanceAssignment.ShowDocument(_obj, document, acquaintanceVersionNumber);
    }
      ...
      else
      {
        ...
        if (isElectronicAcquaintance)
        {
          ...
          if (!Functions.AcquaintanceTask.Remote.IsDocumentVersionReaded(document, acquaintanceVersionNumber))
          {
            ...
            e.AddError(error, _obj.Info.Actions.OpenDocument);
            return;
          }
