...
namespace Sungero.SmartProcessing.Server
{
  partial class BlobFunctions
  {
    ...
    public static IBlob CreateBlob(Structures.Module.IDcsBlob dcsBlob)
    {
      ...
      blob.Created = dcsBlob.Created;
      blob.Modified = dcsBlob.Modified;
    ...
    public virtual Structures.Module.IDcsBlob CreateDcsBlob()
    {
      ...
      if (Functions.Module.GetConfigSettingsTenantOffsetInDateTimeParam())
      {
        dcsBlob.Created = _obj.Created.FromUtcTime();
        dcsBlob.Modified = _obj.Modified.FromUtcTime();
      }
      else
      {
