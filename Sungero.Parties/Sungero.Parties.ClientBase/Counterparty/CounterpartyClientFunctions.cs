...
namespace Sungero.Parties.Client
{
  partial class CounterpartyFunctions
  {
    ...
    public virtual Structures.Counterparty.InvitationInfo ValidateExchangeAction(Sungero.Domain.Client.ExecuteActionArgs args)
    {
      if (!Functions.Counterparty.ValidateTinTrrcBeforeExchange(_obj, args))
      {
        var empty = Structures.Counterparty.InvitationInfo.Create();
        empty.CanDoAction = false;
        return empty;
      }
      
      var info = Structures.Counterparty.InvitationInfo.Create();
      info.CanDoAction = false;
      
      try
      {
        info = Functions.Counterparty.Remote.GetInvitationInfo(_obj);
      }
      catch (Exception ex)
      {
