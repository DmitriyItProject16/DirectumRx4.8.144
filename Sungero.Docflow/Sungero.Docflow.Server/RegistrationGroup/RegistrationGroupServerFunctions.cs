  ...
  partial class RegistrationGroupFunctions
  {
    ...
    public virtual void GrantRegistrationAccessRights()
    {
      if (Users.Current.IncludedIn(Roles.Administrators))
      {
        ...
        if (!Equals(_obj.State.Properties.CanRegisterIncoming.OriginalValue, _obj.CanRegisterIncoming.Value))
          this.GrantAccessRightsOnTypes(_obj.CanRegisterIncoming == true, incomingRights);
        if (!Equals(_obj.State.Properties.CanRegisterOutgoing.OriginalValue, _obj.CanRegisterOutgoing.Value))
          this.GrantAccessRightsOnTypes(_obj.CanRegisterOutgoing == true, outgoingRights);
        if (!Equals(_obj.State.Properties.CanRegisterInternal.OriginalValue, _obj.CanRegisterInternal.Value))
          this.GrantAccessRightsOnTypes(_obj.CanRegisterInternal == true, internalRights);
        if (!Equals(_obj.State.Properties.CanRegisterContractual.OriginalValue, _obj.CanRegisterContractual.Value))
