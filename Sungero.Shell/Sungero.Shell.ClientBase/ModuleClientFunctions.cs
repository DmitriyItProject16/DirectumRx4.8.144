  ...
  public class ModuleFunctions
  {
    ...
    public virtual void OpenAssignmentCompletionReport(Enumeration performer, Enumeration period)
    {
      ...
      else if (performer == Shell.Widgets.AssignmentCompletionGraph.Performer.MyDepartments)
      {
        ...
        Docflow.PublicFunctions.Module.OpenEmployeesAssignmentCompletionReport(null, null, new List<long>(), departmentIds, periodBegin, periodEnd, widgetParameter, true, false, true);
      }
      else if (performer == Shell.Widgets.AssignmentCompletionGraph.Performer.MyDirectDepts)
      {
        var departmentIds = Docflow.PublicFunctions.Module.Remote.GetManagersDepartments();
        Docflow.PublicFunctions.Module.OpenEmployeesAssignmentCompletionReport(null, null, new List<long>(), departmentIds, periodBegin, periodEnd, widgetParameter, false, false, true);
      }
      ...
      {
        Docflow.PublicFunctions.Module.OpenDepartmentsAssignmentCompletionReport(new List<long>(), periodBegin, periodEnd, string.Empty, true, false);
      }
    }
    ...
    /// <param name="period">Параметр "Период".</param>
    [Public]
    public virtual void OpenReportFromEmployeeWidgets(long employeeId, Enumeration period)
    {
      var periodBegin = Functions.Module.GetWidgetBeginPeriod(period);      
    ...
    [Public]
    public virtual void OpenReportFromDepartmentWidgets(long departmentId, List<long> departmentIds, Enumeration performer, Enumeration period, bool sortByAssignmentCompletion)
    {
      var periodBegin = Functions.Module.GetWidgetBeginPeriod(period);
      
      var department = Company.PublicFunctions.Department.Remote.GetDepartment(departmentId);
      
      Docflow.PublicFunctions.Module.OpenEmployeesAssignmentCompletionReport(null, department, new List<long>(), departmentIds, periodBegin, Calendar.UserToday, null, false, false, sortByAssignmentCompletion);
    }
  }
}